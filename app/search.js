...
export function setSearch(q)    { state.q = q.trim().toLowerCase(); apply(); }
export function setSupport(s)   { state.support = s; apply(); }
export function setCategory(c)  { state.category = c; apply(); }
...
export function apply() {
  ...
  for (const sec of sections) {
    ...
    for (const row of sec.querySelectorAll('.row')) {
      ...
      const matchSupport = state.support === 'all' || supported === state.support;
